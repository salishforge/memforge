      ...
      '/memory/{agentId}/query': {
        get: {
          ...
          parameters: [
            ...
            { name: 'snippet_tokens', in: 'query', schema: { type: 'integer', minimum: 1 }, description: 'Reduce each returned memory to the passages most relevant to q, within this per-result token budget. A consolidated memory is a whole conversation while the answer is usually one exchange, so this is typically a large context saving. Lexical and deterministic — no LLM call. Omitted, full content is returned.' },
